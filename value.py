...
# Custom Styling (RTL removed, keeping sizes and weights)
st.markdown(
    """
    <style>
...
# Data Fetching and Calculation Function (Cached for performance)
...
def analyze_stock(
    ...
    try:
        ...
        try:
            ...
            cogs = float(
                income_stmt.loc["Reconciled Cost Of Revenue"].iloc[0]
            )
        ...
        try:
            ...
            operating_income = float(
                income_stmt.loc["Operating Revenue"].iloc[0]
        ...
        try:
            ...
            tax_provision = float(
                income_stmt.loc["Income Tax Expense"].iloc[0]
            )
        ...
        total_debt = (
            ...
            else float(balance_sheet.loc["Total Debt"].iloc[0])
        )
        ...
        for i in range(num_years):
            try:
                ...
                t_rate = (
                    (t_prov / p_inc) if p_inc > 0 and t_prov >= 0 else 0.21
                )
                h_nopat = op_inc * (1 - t_rate)

                try:
                    st_d = float(balance_sheet.loc["Current Debt"].iloc[i])
                except KeyError:
                    st_d = 0.0
                try:
                    lt_d = float(balance_sheet.loc["Long Term Debt"].iloc[i])
                except KeyError:
                    lt_d = 0.0
                tot_d = (
                    (st_d + lt_d)
                    if (st_d > 0 or lt_d > 0)
                    else float(balance_sheet.loc["Total Debt"].iloc[i])
                )
                eq = float(balance_sheet.loc["Stockholders Equity"].iloc[i])
                c_sh = float(
                    balance_sheet.loc[
                        "Cash Cash Equivalents And Short Term Investments"
                    ].iloc[i]
        ...
        equity_value = (
            total_pv_phase_1 + pv_terminal_value + cash - total_debt
        )
...
st.caption("Based on Warren Buffett's valuation principles, Cash Flow Analysis, and DCF Modeling")
...
with tab1:
    ...
    if selected_ticker:
        ...
        if data:
            ...
            m2.metric("Intrinsic Value per Share", f"${data['intrinsic_value']}")

            mos_color = "normal" if data["margin_of_safety"] >= 20 else "inverse"
            m3.metric(
            ...
            # 2. Detailed Breakdowns (7 Sections)
            ...
            with col_left:
                ...
                with st.expander(
                    ...
                    st.write(f"**Historical Avg ROIC:** {data['avg_roic']}%")
                    st.write(
                ...
                with st.expander("3. Profitability & Margins"):
                    ...
                    st.write(
                        f"**Gross Margin:** {data['gross_margin']}%"
                    )
                    st.write(
            ...
            with col_right:
                ...
                with st.expander("5. Return on Invested Capital (ROIC vs WACC)"):
                    st.write(f"**ROIC:** {data['roic']}%")
                    st.write(f"**WACC:** {data['wacc']}%")
                    st.write(
                        f"**Economic Spread:** {data['spread']}%"
                    )

                with st.expander(
                    "6. Cash Flow & Earnings Quality (FCFCR)", expanded=True
                ):
                    st.write(
                    ...
                    if data["fcfcr"] >= 1.0:
                        ...
                        st.error("🚨 Warning: Paper profits lacking cash backing")

                with st.expander(
                    "7. Free Cash Flow Yield (FCF Yield)", expanded=True
                ):
                    st.write(
                        f"**FCF Yield:** {data['fcf_yield']}%"
                    )
                    if data["fcf_yield"] >= 8.0:
                        st.success("🟢 Highly undervalued and cash-generating")
                    elif data["fcf_yield"] >= 3.0:
                        st.info("🟡 Fairly priced")
                    else:
                        st.error("🔴 Expensive relative to cash generation")
